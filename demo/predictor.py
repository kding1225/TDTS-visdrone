...
from fcos_core.utils.colormap import random_color
...
class COCODemo(object):
    ...
    def __init__(
        ...
        self.colormap = [random_color(True, 255, i) for i in range(len(self.CATEGORIES))]
    ...
    def run_det_on_opencv_image_plt(self, image, filename, dpi=300):
        predictions = self.compute_prediction(image)
        top_predictions = self.select_top_predictions(predictions)
        ...
        colors = [self.colormap[i] for i in labels]  # RGB
        ...
        for box, label, score, color in zip(boxes, labels, scores, colors):
            ...
            display_txt = '{}: {:.2f}'.format(label, score)
            currentAxis.add_patch(
                plt.Rectangle(top_left,
            ...
            currentAxis.text(
                *top_left,
                display_txt,
                fontsize=4,
                bbox={'facecolor': color, 'alpha': 0.5, 'pad':0, 'clip_on': True, 'linewidth':1}
            )
        plt.axis('off')
...
class VisDroneDemo(COCODemo):
    ...
    def select_top_predictions(self, predictions):
        ...
        scores = predictions.get_field("scores")
        labels = predictions.get_field("labels")
        thresholds = self.confidence_thresholds_for_classes[(labels).long()]
        keep = torch.nonzero(scores > thresholds).squeeze(1)
        predictions = predictions[keep]
        scores = predictions.get_field("scores")
        _, idx = scores.sort(0, descending=True)
