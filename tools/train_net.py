...
from fcos_core.utils.tensorboardX import SummaryWriterX
from fcos_core.data.datasets.category import get_category
from fcos_core.utils.metric_logger import MetricLogger
from fcos_core.utils.flops_counter import params_to_string
...
def train(cfg, local_rank, distributed):
    ...
    print('# parameters totally: ' + params_to_string(num_parameters, 'M'))
    ...
    if 0 < vis_period < cfg.SOLVER.MAX_ITER:
        visualizer = SummaryWriterX(
            cfg.VISUALIZE.DIR+'/'+cfg.VISUALIZE.ENV,
            cfg.VISUALIZE.ENV,
            vis_period,
            20,
            get_category(cfg.DATASETS.TRAIN[0])
        )
    else:
        visualizer = None

    meters = MetricLogger(delimiter="  ", save_dir=os.path.join(output_dir, 'meters.json'))
...
def run_test(cfg, model, distributed):
    ...
    for output_folder, dataset_name, data_loader_val in zip(output_folders, dataset_names, data_loaders_val):
        inference(
            ...
            iou_types=iou_types,
            box_only=False if
            cfg.MODEL.FCOS_VISDRONE_ON or
            cfg.MODEL.TDTS_VISDRONE_ON or
            cfg.MODEL.RETINA_VISDRONE_ON
