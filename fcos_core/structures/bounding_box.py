...
class BoxList(object):
    ...
    def resize(self, size, *args, **kwargs):
        ...
        the size field will be changed
    ...
    def export_to_image(self, image, save_path, dpi=100):
        """
        plot boxes to an image, and export using given path
        image: tensor, c*h*w
        """
        import numpy as np
        ...
        boxes = self.bbox.cpu().data.numpy()
        ...
        for i in range(len(boxes)):
            pt = boxes[i][:4]
            label_name = str(int(labels[i])) if len(labels) > 0 else 'C'
        ...
        plt.xlim([0, np.max(boxes[:, 2])])
        plt.ylim([np.max(boxes[:, 3]), 0])
