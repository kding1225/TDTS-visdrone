...
from fcos_core.modeling.rpn.fcos_visdrone.inference import make_fcos_postprocessor as fcos_postprocessor
from fcos_core.modeling.rpn.tdts_visdrone.inference import make_fcos_postprocessor as tdts_postprocessor
...
def im_detect_bbox_aug(model, images, device):
    ...
    assert cfg.MODEL.FCOS_ON or cfg.MODEL.FCOSMFPN_ON, \
        "The multi-scale testing only supports FCOS and FCOS_MFPN detector"
    ...
    if cfg.MODEL.FCOS_VISDRONE_ON:
        post_processor = fcos_postprocessor(cfg)
    elif cfg.MODEL.TDTS_VISDRONE_ON:
        post_processor = tdts_postprocessor(cfg)
    else:
        raise NotImplementedError
    results = post_processor.select_over_all_levels(boxlists, cfg.MODEL.DEVICE)
