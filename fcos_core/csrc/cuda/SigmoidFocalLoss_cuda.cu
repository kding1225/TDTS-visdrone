...
__global__ void SigmoidFocalLossForward(const int nthreads, 
  ...
  CUDA_1D_KERNEL_LOOP(i, nthreads) {
    ...
    // p**gamma * log(1-p), x>0 is better in expf(-x), avoiding overflow
  ...
  CUDA_1D_KERNEL_LOOP(i, nthreads) {
    ...
    T term2 = powf(p, gamma) *
                  ((-1. * logits[i] * (logits[i] >= 0) -
                      logf(1. + expf(logits[i] - 2. * logits[i] * (logits[i] >= 0)))) *
                      (1. - p) * gamma - p);
    d_logits[i] = 0.0;  // the gradients of this layer outputs w.r.t. this layer input
    d_logits[i] += -c1 * term1 * zp;
    d_logits[i] += -c2 * term2 * zn;
    d_logits[i] = d_logits[i] * d_losses[i];  // d_losses come from top layer
...
at::Tensor SigmoidFocalLoss_forward_cuda(
  ...
  // std::cout<<"logits: "<<logits.options()<<std::endl;
  // std::cout<<"targets: "<<targets.options()<<std::endl;
...
at::Tensor SigmoidFocalLoss_backward_cuda(
		const at::Tensor& logits,
                const at::Tensor& targets,
		const at::Tensor& d_losses, // top gradients, partial tol. loss/partial loss tensor
		const int num_classes, 
		const float gamma, 
		const float alpha) {
  AT_ASSERTM(logits.type().is_cuda(), "logits must be a CUDA tensor");
