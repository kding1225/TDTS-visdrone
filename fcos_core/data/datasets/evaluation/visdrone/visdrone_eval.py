...
from .boxes_filter import filter_boxes
...
def do_visdrone_evaluation(
    ...
    # filter gt boxes
    if filter_opt:
        logger.info("Filtering boxes use filter_opt: %s"%filter_opt)
        dataset.coco = filter_boxes(dataset.coco, filter_opt)
...
def prepare_for_coco_detection(predictions, dataset):
    ...
    for image_id, prediction in enumerate(predictions):
        ...
        mapped_labels = [dataset.contiguous_category_id_to_json_id[i+1] for i in labels]
