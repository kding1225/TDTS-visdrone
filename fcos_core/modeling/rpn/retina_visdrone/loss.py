...
class RetinaNetLossComputation(RPNLossComputation):
    ...
    def __call__(self, anchors, box_cls, box_regression, targets):
        ...
        labels = labels.int()
        keep = (labels != 1) & (labels >= 0)
        new_labels = (labels - 1).clamp(min=0)
...
def make_retinanet_loss_evaluator(cfg, box_coder):
    matcher = Matcher(
        cfg.MODEL.RETINA_VISDRONE.FG_IOU_THRESHOLD,
        cfg.MODEL.RETINA_VISDRONE.BG_IOU_THRESHOLD,
        allow_low_quality_matches=True,
    )
    sigmoid_focal_loss = SigmoidFocalLoss(
        cfg.MODEL.RETINA_VISDRONE.LOSS_GAMMA,
        cfg.MODEL.RETINA_VISDRONE.LOSS_ALPHA
    )
    ...
    loss_evaluator = RetinaNetLossComputation(
        ...
        sigmoid_focal_loss,
        bbox_reg_beta = cfg.MODEL.RETINA_VISDRONE.BBOX_REG_BETA,
        regress_norm = cfg.MODEL.RETINA_VISDRONE.BBOX_REG_WEIGHT,
    )
