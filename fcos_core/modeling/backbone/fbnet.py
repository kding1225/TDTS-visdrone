...
class FBNet(nn.Module):
    ...
    used for FPN
    """
    def __init__(self, cfg, dim_in=3):
        super(FBNet, self).__init__()
        builder, arch_def = create_builder(cfg)
        self.first = builder.add_first(arch_def["first"], dim_in=dim_in)
        trunk_cfg = _get_trunk_cfg(arch_def)
        self.stages = builder.add_blocks(trunk_cfg["stages"])
        self.return_features_indices = [2, 5, 12, 16]
        self.return_features_num_channels = [24, 32, 96, 240]
    ...
    def forward(self, x):
        """
        need to collect features: C2, C3, C4, C5
        """
        y = self.first(x)
        ret = []
