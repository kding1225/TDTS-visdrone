...
# ResNet-50-FPN (excluding the last stage)
...
class ResNet(nn.Module):
    ...
    def forward_dummy(self, x):
        self.__flops__ = 0
        outputs = []
        x = self.stem.forward_dummy(x)
        self.__flops__ += self.stem.__flops__
        for stage_name in self.stages:
            stage = getattr(self, stage_name)
            for layer in stage:
...
class Bottleneck(nn.Module):
    ...
    def forward_dummy(self, x):
        ...
        out += identity
        out = F.relu_(out)
        self.__flops__ += identity.numel() * 2
        return out
...
class BaseStem(nn.Module):
    ...
    def forward_dummy(self, x):
        ...
        if not self.is_fuse:
            ...
            self.__flops__ += flops
        x = F.relu_(x)
        self.__flops__ += x.numel()

        x_ = F.max_pool2d(x, kernel_size=3, stride=2, padding=1)
...
_STAGE_SPECS = Registry({
    ...
    "R-50-FPN-234": ResNet50FPNStagesTo4,
    "R-101-FPN-234": ResNet101FPNStagesTo4,
