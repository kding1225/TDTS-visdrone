...
class ResNet50Conv5ROIFeatureExtractor(nn.Module):
    ...
    def forward_dummy(self, x, proposals):
        self.__flops__ = 0
        x = self.pooler(x, proposals)  # todo: count self.pooler flops
...
class FPN2MLPFeatureExtractor(nn.Module):
    ...
    def forward_dummy(self, x, proposals):
        self.__flops__ = 0
        x = self.pooler(x, proposals)  # todo: add self.pooler flops
        x = x.view(x.size(0), -1)

        x, flops = count_apply_module(self.fc6, [x])
        self.__flops__ += flops
        x = F.relu(x)

        x, flops = count_apply_module(self.fc7, [x])
        self.__flops__ += flops
        x = F.relu(x)
...
class FPNXconv1fcFeatureExtractor(nn.Module):
    ...
    def forward_dummy(self, x, proposals):
        ...
        for layer in self.xconvs:
            x, flops = count_apply_module(layer, [x])
            self.__flops__ += flops
        x = x.view(x.size(0), -1)
